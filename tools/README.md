...
the clone. Either way, **verify** it is live — that is section C. You only apply
...
5. **Build and run** (the Dockerfile already pins `protobuf==3.20.3`):
   ...
   sudo docker run -d --restart unless-stopped --name nfcgate-server \
     -p 5566:5566 nfcgate-server
   ```

   `--restart unless-stopped` survives reboots and SSH logout. **No arguments =
   no plugins = fast mode.** For debugging, re-run with `nfcgate-server log -v`
   and watch `sudo docker logs -f nfcgate-server` — but both cost latency, so
   drop them when you're done.

   > **Re-patching later?** The Dockerfile does `COPY server.py` at *build* time,
   > so `docker restart` does **not** pick up code changes. After (re)applying the
   > patch you must `docker build` again and recreate the container
   > (`sudo docker rm -f nfcgate-server` then the `docker run` above).

*(No Docker? Run `server.py` under systemd instead — create a venv with
`protobuf==3.20.3` and a `Type=simple` unit whose `ExecStart` is
`.venv/bin/python server.py`. The patch is still mandatory. Full unit file in the
Spanish guide.)*

### C. Verify the patch is actually live
...
# 2. Inside the *running* container (Docker bakes server.py at build time):
sudo docker exec nfcgate-server grep -c TCP_NODELAY /srv/server.py   # must print 2

# 3. On the wire, from your machine, no hardware needed (conclusive):
bombercat testserver verify VPS_IP 5566        # -> RESULT: PATCH ACTIVE
```

Check 2 is the one people skip and the one that bites: a stale image keeps
serving the old `server.py` even after you patch the file. Check 3 measures what
the server *does* (frames arriving in one segment vs two) and also reports the
relay round-trip.

### D. Point the boards at it, then run
...
> Tailscale) or restrict the source by firewall.
